...
type AssetLibraryGroup struct {
	...
	Mappings    []AssetLibraryGroupUpstream `json:"mappings" gorm:"foreignKey:GroupId"`
}
...
type AssetLibraryAsset struct {
	...
	Mappings    []AssetLibraryAssetUpstream `json:"mappings" gorm:"foreignKey:AssetId"`
}

// AssetLibraryGroupUpstream maps a local group to its counterpart on one
// configured upstream. UpstreamGroupId is empty for OpenAI-style upstreams,
// which have no group concept.
type AssetLibraryGroupUpstream struct {
	Id              int64  `json:"id" gorm:"primaryKey"`
	GroupId         int64  `json:"group_id" gorm:"uniqueIndex:idx_asset_group_upstream;not null"`
	UserId          int    `json:"-" gorm:"index;not null"`
	UpstreamId      int64  `json:"upstream_id" gorm:"uniqueIndex:idx_asset_group_upstream;index;not null"`
	...
	CreatedTime     int64  `json:"created_time" gorm:"bigint"`
	UpdatedTime     int64  `json:"updated_time" gorm:"bigint"`
	UpstreamName    string `json:"upstream_name" gorm:"-"`
}

type AssetLibraryAssetUpstream struct {
	Id              int64  `json:"id" gorm:"primaryKey"`
	AssetId         int64  `json:"asset_id" gorm:"uniqueIndex:idx_asset_upstream;not null"`
	GroupUpstreamId int64  `json:"group_upstream_id" gorm:"index;not null"`
	UserId          int    `json:"-" gorm:"index;not null"`
	UpstreamId      int64  `json:"upstream_id" gorm:"uniqueIndex:idx_asset_upstream;index;not null"`
...
func SaveAssetLibraryGroupUpstream(mapping *AssetLibraryGroupUpstream) error {
	now := time.Now().Unix()
	...
	return DB.Clauses(clause.OnConflict{
		Columns: []clause.Column{{Name: "group_id"}, {Name: "upstream_id"}},
...
func SaveAssetLibraryAssetUpstream(mapping *AssetLibraryAssetUpstream) error {
	now := time.Now().Unix()
	if mapping.CreatedTime == 0 {
		mapping.CreatedTime = now
	}
	mapping.UpdatedTime = now
	return DB.Clauses(clause.OnConflict{
		Columns: []clause.Column{{Name: "asset_id"}, {Name: "upstream_id"}},
		DoUpdates: clause.AssignmentColumns([]string{
			"group_upstream_id", "upstream_asset_id", "asset_url", "status",
...
func DeleteAssetLibraryGroup(userId int, id int64) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		...
		if len(assetIds) > 0 {
			if err := tx.Where("asset_id IN ?", assetIds).Delete(&AssetLibraryAssetUpstream{}).Error; err != nil {
				return err
			}
		}
		...
		if err := tx.Where("group_id = ? AND user_id = ?", id, userId).Delete(&AssetLibraryGroupUpstream{}).Error; err != nil {
			return err
		}
...
func DeleteAssetLibraryAsset(userId int, groupId int64, assetId int64) error {
	return DB.Transaction(func(tx *gorm.DB) error {
		if err := tx.Where("asset_id = ? AND user_id = ?", assetId, userId).Delete(&AssetLibraryAssetUpstream{}).Error; err != nil {
			return err
		}
