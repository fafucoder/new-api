...
// responsesViaChatCompletions downgrades an OpenAI Responses request to a Chat Completions request
// for upstreams that only support /v1/chat/completions, then rewrites the upstream chat response
// back into the Responses format. It mirrors chatCompletionsViaResponses (the reverse direction).
func responsesViaChatCompletions(c *gin.Context, info *relaycommon.RelayInfo, adaptor channel.Adaptor, request *dto.OpenAIResponsesRequest) (*dto.Usage, *types.NewAPIError) {
	...
	// Ensure upstream returns usage on streaming so billing stays accurate.
	if chatReq.Stream != nil && *chatReq.Stream && info.SupportStreamOptions {
