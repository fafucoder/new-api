...
// 把内部档位标签（如 "video|720p|0|default" / "video|480p|1|s|default"）翻译成
// 可读文案："视频 720p · 无参考视频 · 默认档" / "视频 480p · 含参考视频 · 按秒 · 默认档"。
// 非视频标签保持原样。
function formatMatchedTierLabel(rawLabel) {
  ...
  const isPerSecond = rest.includes('s');
...
export function renderTieredModelPrice(opts) {
  ...
  const lines = [
    buildBillingText('命中档位：{{tier}}', {
      tier: formatMatchedTierLabel(matchedTier) || tier.label,
    }),
    ...priceLines
      .filter(([field]) => tier[field] > 0)
      .map(([field, label]) =>
        buildBillingPriceText(
          `${relabel(field, label)}：{{symbol}}{{price}} / 1M tokens`,
          { symbol, usdAmount: tier[field], rate },
        ),
      ),
  ];
