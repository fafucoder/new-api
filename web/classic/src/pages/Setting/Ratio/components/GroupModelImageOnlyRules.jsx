...
function flattenRules(nested) {
  ...
  for (const [group, inner] of Object.entries(nested)) {
    ...
    for (const [modelName, imageOnly] of Object.entries(inner)) {
      rules.push({
        _id: uid(),
        group,
        modelName,
...
function nestRules(rules) {
  const result = {};
  rules.forEach(({ group, modelName, imageOnly }) => {
    if (!group || !modelName) return;
    if (!result[group]) result[group] = {};
    result[group][modelName] = !!imageOnly;
  });
  return result;
}

export function serializeGroupModelImageOnly(rules) {
          ...
          {items.map((rule) => (
            ...
            >
              ...
              <div className='flex items-center gap-1' style={{ width: 120 }}>
                <Switch
                  size='small'
                  checked={rule.imageOnly}
                  onChange={(v) => onUpdate(rule._id, 'imageOnly', v)}
                />
                <Text type='tertiary' size='small'>
                  {rule.imageOnly ? t('生图') : t('否')}
                </Text>
              </div>
              <Popconfirm
                title={t('确认删除该规则？')}
...
export default function GroupModelImageOnlyRules({
  value,
  groupNames = [],
  onChange,
}) {
  ...
  const emitChange = useCallback(
    (newRules) => {
      setRules(newRules);
      onChange?.(serializeGroupModelImageOnly(newRules));
    },
    [onChange],
  );
  ...
  const addRuleToGroup = useCallback(
    (groupName) => {
      emitChange([
        ...rules,
        { _id: uid(), group: groupName, modelName: '', imageOnly: true },
      ]);
    },
    [rules, emitChange],
  );
  ...
  const addNewGroup = useCallback(() => {
    ...
    emitChange([
      ...rules,
      { _id: uid(), group: name, modelName: '', imageOnly: true },
    ]);
    setNewGroupName('');
